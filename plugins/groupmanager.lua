...
if matches[1] == "pin" and is_mod(msg) then
...
if matches[1] == "promod" and is_owner(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
      message_id_ = msg.reply_to_message_id_
    }, action_by_reply, {chat_id=msg.chat_id_,cmd="promod"})
  end
  if matches[2] and string.match(matches[2], '^%d+$') then
tdcli_function ({
    ID = "GetUser",
    user_id_ = matches[2],
  }, action_by_id, {chat_id=msg.chat_id_,user_id=matches[2],cmd="promod"})
    end
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdcli_function ({
      ID = "SearchPublicChat",
      username_ = matches[2]
    }, action_by_username, {chat_id=msg.chat_id_,username=matches[2],cmd="promod"})
      end
   end
if matches[1] == "demod" and is_owner(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
 tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
      message_id_ = msg.reply_to_message_id_
    }, action_by_reply, {chat_id=msg.chat_id_,cmd="demod"})
  end
  if matches[2] and string.match(matches[2], '^%d+$') then
tdcli_function ({
    ID = "GetUser",
    user_id_ = matches[2],
  }, action_by_id, {chat_id=msg.chat_id_,user_id=matches[2],cmd="demod"})
    end
  if matches[2] and not string.match(matches[2], '^%d+$') then
    tdcli_function ({
      ID = "SearchPublicChat",
      username_ = matches[2]
    }, action_by_username, {chat_id=msg.chat_id_,username=matches[2],cmd="demod"})
      end
   end

if matches[1] == "k" and is_mod(msg) then
local target = msg.chat_id_
...
if matches[2] == "men" then
return lock_mention(msg, data, target)
end
if matches[2] == "ar" then
return lock_arabic(msg, data, target)
end
if matches[2] == "ed" then
...
if matches[1] == "n" and is_mod(msg) then
local target = msg.chat_id_
if matches[2] == "lk" then
...
if matches[1] == "sg" and is_owner(msg) then
...
if matches[1] == "h1" and is_mod(msg) then
if not lang then
text = [[
*promod* `رفع ادمن` 
*demod* `ازاله ادمن` 
*setflood* `[1-50]` `وضع تكرار بالطرد`
*setfltime* `[1-10]` `وضع تكرار بالتحذير`
*nsil* `لالغاء كتم العضو` 
*sil* `لكتم عضو` 
*kick* `حظر عضو` 
*ban* `حظر عضو` 
*nban* `الغاء حظر العضو` 
*id* `لاظهار الايدي [بالرد] `
*pin* `تثبيت رساله!`
*unpin* `الغاء تثبيت الرساله!`
*locklist* `اظهار الاعدادات`
*mutelist* `اظهار الاعدادات الثانيه`
*sillist* `اظهار المكتومين`
...
*bad* `منع كلمه` 
*nbad* `الغا منع كلمه` 
*del badlist* `حذف الكلمات الممنوعه` 
*badlist* `اظهار الكلمات الممنوعه` 
*gpinfo* `اظهار معلومات المجموعه`
*link* `اظهار الرابط`
]]
end
return text
...
if matches[1] == "h2" and is_mod(msg) then
if not lang then
text = [[
*k* `للقفل`
*n* `للفتح`
...
if matches[1] == "h3" and is_mod(msg) then
if not lang then
text = [[
*clean* `مع الاوامر لحذفها`
*____________________*
*| bans |* `المحظورين`
*| mods |* `الادمنيه`
*| bots |* `بوتات التفليش`
...
if not lang then
text = [[
`اهلا وسهلا بك في بوت تشاكي`

`تعمل جميع الاوامر ب[#@!] او بدونهن`

`هناك`  *3* `اوامر لعرضها`

*h1* `لعرض اوامر الادمنيه`

*h2* `لعرض اوامر الحمايه`

*h3* `لعرض اوامر المجموعه`
]]
end
return text
end

if matches[1] == "8" and is_mod(msg) then
if not lang then
text = [[
...
--------------------- Welcome -----------------------
	if matches[1] == "wlc" and is_mod(msg) then
	...
	if matches[1] == "swlc" and matches[2] and is_mod(msg) then
